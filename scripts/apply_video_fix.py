...
with open("webapp/src/components/Hero.tsx", "w", encoding="utf-8") as f:
    hero_code = r"""import { useEffect, useMemo, useRef } from "react";
...
export function Hero({
  ...
  return (
    ...
    >
      ...
      {/* ── 4. Main Centered 100% Pure Optical Crystal Glass Showcase ── */}
      <motion.div
        style={{
          width: "100%",
          maxWidth: "650px",
      ...
      >
        <div
          style={{
            ...
            boxShadow: "inset 0 1px 1px rgba(255, 255, 255, 0.2), 0 12px 36px rgba(0, 0, 0, 0.2)",
            display: "flex",
            flexDirection: "column",
            alignItems: "center",
            textAlign: "center",
...
# 2. Update Rail.tsx
with open("webapp/src/components/Rail.tsx", "r", encoding="utf-8") as f:
    rail_content = f.read()

# Add HERO_VIDEOS import
if 'import { HERO_VIDEOS }' not in rail_content:
    rail_content = rail_content.replace(
        'import {',
        'import { HERO_VIDEOS } from "@/components/Hero";\nimport {'
    )

# Update RailProps interface
rail_content = re.sub(
    r'export interface RailProps \{([^}]+)\}',
    r'''export interface RailProps {
  feeds: FeedStatus[];
  stamp: string;
  onRefresh: () => void;
...
# Update Rail function signature
rail_content = re.sub(
    r'export function Rail\(\{([^}]+)\}: RailProps\)',
    r'export function Rail({ feeds, stamp, onRefresh, currentPage, onPageChange, activeVideo = 0, onVideoChange }: RailProps)',
    rail_content
)

# Insert the theme switcher cleanly into <header className="rail">
new_header_middle = r"""        </div>
...
print("All components cleanly updated with integrated video switcher and instant autoplay fix!")
